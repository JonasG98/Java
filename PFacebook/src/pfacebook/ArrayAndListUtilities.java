...
public class ArrayAndListUtilities {
	...
	{
		//ArrayAndListUtilities.bubbleSortv1(words);
		int left = 0;
	...
	public static void bubbleSortv1(Friend[] words)
	{
		boolean sorted = false;
		while (sorted == false)
		{
			...
			{
				if(words[j].getlastName().compareTo(words[j+1].getlastName()) > 0)
				{
				Friend temp = words[j + 1];
				words[j+1] = words[j];
				words[j] = temp;
				sorted = false;
				}
